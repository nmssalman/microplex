...
public sealed class EmailController(ApplicationDbContext db, EmailSender emailSender) : Controller
{
    ...
    {
        var html = LowEmailCreditEmailTemplateBuilder.Build(LowEmailCreditEmailTemplateBuilder.SampleCompanyName, LowEmailCreditEmailTemplateBuilder.SampleBalance);
    ...
    public async Task<IActionResult> AddCredit(Guid clientId, int amount)
    {
        ...
        client.EmailCredits += amount;
        await db.SaveChangesAsync();
        TempData["Success"] = $"{amount} credits added to {client.CompanyName}. New balance: {client.EmailCredits}.";
        return RedirectToAction(nameof(Index));
    }
